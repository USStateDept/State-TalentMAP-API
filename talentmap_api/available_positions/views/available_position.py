...
class FavoritesCSVView(APIView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        if len(aps) > 0 and request.query_params.get('exclude_available') != 'true':
            pos_nums = ','.join(aps)
            apdata = services.get_available_positions(QueryDict(f"id={pos_nums}&limit=500&page=1"), request.META['HTTP_JWT'])
        ...
        if len(pvs) > 0 and request.query_params.get('exclude_projected') != 'true':
            pos_nums = ','.join(pvs)
            pvdata = pvservices.get_projected_vacancies(QueryDict(f"id={pos_nums}&limit=500&page=1"), request.META['HTTP_JWT'])
