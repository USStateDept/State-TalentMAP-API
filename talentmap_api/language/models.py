...
class Qualification(StaticRepresentationModel):
    ...
    def get_or_create_by_codes(language_code, reading_proficiency_code, spoken_proficiency_code):
        ...
        if language.count() != 1 or reading_proficiency.count() != 1 or spoken_proficiency.count() != 1:
            logging.getLogger(__name__).warn(f"Tried to create language qualification, but failed: {language_code}
                                             ({language.count()}) {reading_proficiency_code}({reading_proficiency.count()}) {spoken_proficiency_code}({spoken_proficiency.count()})")
            return None, False
