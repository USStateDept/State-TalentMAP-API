...
def test_ordering_position_fixture():
    ...
    mommy.make('bidding.CyclePosition', position=pos1, bidcycle=bc)
    mommy.make('bidding.CyclePosition', position=pos2, bidcycle=bc)


@pytest.mark.django_db()
@pytest.mark.usefixtures("test_ordering_position_fixture")
def test_regular_ordering(client):
    response = client.get('/api/v1/position/?ordering=position__position_number')
    assert response.status_code == status.HTTP_200_OK

    assert response.data["results"][0]["position"]["position_number"] == "1234"

    response = client.get('/api/v1/position/?ordering=-position__position_number')

    assert response.status_code == status.HTTP_200_OK

    assert response.data["results"][0]["position"]["position_number"] == "5678"

@pytest.mark.django_db()
@pytest.mark.usefixtures("test_ordering_position_fixture")
def test_nested_ordering(client):
    response = client.get('/api/v1/position/?ordering=position__post__has_service_needs_differential')
    assert response.status_code == status.HTTP_200_OK
